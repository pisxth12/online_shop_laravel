        ...
        <div class="block">
            ...
            @foreach ($cartItems as $cartItem)
                ...
                <div class="media product-card">
                    <a class="pull-left" href="{{ route('product.detail', $cartItem->id) }}">
                        <img class="media-object" width="80"
                             src="{{ $cartItem->attributes->image 
                                 ? asset('uploads/product/' . $cartItem->attributes->image) 
                                 : asset('uploads/product/default.png') }}" 
                             alt="{{ $cartItem->name }}">
                    </a>
                    <div class="media-body">
                        <h4 class="media-heading d-flex justify-content-between">
                            {{-- Brand Name if stored in attributes --}}
                            @if(isset($cartItem->attributes->brand_name))
                                <a href="#">{{ $cartItem->attributes->brand_name }}</a>
                            @endif
                            <a href="{{ route('product.detail', $cartItem->id) }}">{{ $cartItem->name }}</a>
                        </h4>
                        <p class="price">{{ $cartItem->quantity }} x ${{ number_format($cartItem->price, 2) }} = ${{ number_format($total_price, 2) }}</p>
                        <a href="{{ route('cart.remove.from.cart', $cartItem->id) }}" class="remove text-danger">Remove</a>
                    </div>
                </div>
            @endforeach
            ...
            <div class="verified-icon">
                <img src="{{ asset('front-end/asset/images/shop/verified.png') }}">
            </div>
        </div>
    </div>
            </div>
